...
import { getStoredUser, getPosts, deletePost, type Post } from "@/lib/api";
    ...
    <div className="max-w-5xl mx-auto">
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 font-sans text-sm text-ravok-slate">View all posts</p>
        </div>
                  ...
                  {posts.map((post, i) => (
                    <tr key={post.id} className="transition-colors hover:bg-white/5">
                      ...
                      <td className="px-4 py-3">
                        {post.published_at ? (
                          <span className="text-ravok-slate">
                            {new Date(post.created_at).toLocaleDateString()}
                          </span>
                        ) : (
                          <span className="inline-block rounded bg-ravok-gold/20 px-2 py-0.5 text-xs font-medium uppercase tracking-wider text-ravok-gold">
                            Draft
                          </span>
                        )}
                      </td>
