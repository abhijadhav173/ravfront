...
export default function AdminInvestorsAllPage() {
  ...
  async function loadPage(p: number) {
    try {
      const res = await getUsers({ page: p, role: "investor" });
      setUsers(res.data);
      setPage(p);
      setLastPage(res.last_page);
    } catch {
    ...
    <div className="max-w-5xl mx-auto">
      <div className="mb-8">
        ...
        <p className="mt-1 font-sans text-sm text-ravok-slate">
          All investors (approved, pending, or rejected)
        </p>
      </div>
      ...
      <div className="overflow-hidden rounded-lg border border-white/10 bg-black/40 shadow-lg">
        {loading ? (
          ...
          <div className="py-16 text-center font-sans text-ravok-slate">No investors yet.</div>
        ) : (
          <>
