...
export default function AdminInvestorsRequestsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    getUsers({ role: "investor", status: "pending" })
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="overflow-hidden rounded-lg border border-white/10 bg-black/40 shadow-lg">
        {loading ? (
          ...
          <div className="py-16 text-center font-sans text-ravok-slate">No pending requests.</div>
        ) : (
              ...
              <tbody className="divide-y divide-white/5">
                {users.map((u, i) => (
                  <tr key={u.id} className="transition-colors hover:bg-white/5">
                    <td className="px-4 py-3 text-ravok-slate">{i + 1}</td>
                    <td className="px-4 py-3 font-medium text-white">{u.name}</td>
                    <td className="px-4 py-3 text-ravok-slate">{u.email}</td>
                    <td className="px-4 py-3">
                      <span className="inline-block rounded bg-ravok-gold/20 px-2 py-0.5 text-xs font-medium uppercase tracking-wider text-ravok-gold">pending</span>
                    </td>
